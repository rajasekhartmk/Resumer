...
mypath = 'C:/Users/RAJASEKHARTMK/Downloads/check/New folder'  # enter your path here where you saved the resumes
...
def create_profile(file):
    ...
    keyword_dict = pd.read_csv('C:/Users/RAJASEKHARTMK/Downloads/res_excel/res.csv')
    ...
    datamodel_dev = [nlp(text) for text in keyword_dict['DataModeler–DevEngineer'].dropna(axis=0)]
    datatester_dev = [nlp(text) for text in keyword_dict['DataTester–DevOps'].dropna(axis=0)]
    ...
    matcher.add('DataModeler–DevEngineer', None, *datamodel_dev)
    matcher.add('DataTester–DevOps', None, *datatester_dev)
    doc = nlp(text)
    ...
    df = pd.read_csv(StringIO(keywords.lower()), names=['Keywords_List'])
